...
// ehall_grade 成绩页：整条链同步注入——core 在 chart-amd-guard 之前，提供让页面 RequireJS
// 先就绪的延迟，guard 才能正确 nil define.amd 让 Chart.js 全局加载；同步也保住面板位置。
// 可视化 / 查询的开关各自在脚本内用 pjw.featureOn 自检（featureOn 按需读 meta）。
if (pjw_mode == "ehall_grade") {
  injectScripts([
    "js/common/core.js",
...
(async () => {
  const PP_SETTINGS_KEY = "potatoplus_settings";
  let settings = {};
  try { settings = (await browser.storage.local.get(PP_SETTINGS_KEY))[PP_SETTINGS_KEY] || {}; } catch (_) {}

  // Expose mode + settings via meta tags (core.js reads these in its IIFE)
  ...
  sMeta.setAttribute("name", "pjw-settings");
  sMeta.setAttribute("content", JSON.stringify(settings));
  document.documentElement.appendChild(sMeta);

  // Conditional script injection based on current page
  // Scripts are loaded sequentially to guarantee dependency order
  if (pjw_mode == "portal") {
    if (settings["portal.quick_login"] === false) return;
    ...
    if (settings["ehall.eval"] === false) return;
    ...
    if (settings["authserver.beautify"] === false) return;
    // 美化始终开启：隐藏原始页面，注入自定义登录界面；点“登录”时无 UI 自动完成滑块验证。
    ...
    // 成绩页整条链已在上方同步注入（chart-amd-guard 时序敏感、不能 await）；此处不重复注入。
  
  } else if (pjw_mode == "ehall_home") {
    if (settings["ehall.home_cards"] === false) return;
    // ehall 首页
    injectScripts([
      "js/common/core.js",
    ...
    // AMS 作业提交系统 UI 美化（站点级开关由 background 动态注册控制）
    injectScripts([
      "js/common/core.js",
      "js/ams/ams.js",
    ]);
